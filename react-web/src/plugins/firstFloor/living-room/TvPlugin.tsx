...
export const TvPlugin: FC<Props> = () => {
  const theme = useCoreTheme();
  const classes = useStyles(theme);
  const [on, setOn] = useState(true);
  const [clicked, set] = useState(false)

  var Icon = IconRegistry['tvIcon'];
  const onPowerChange = (on: boolean) => {
    setOn(!on);
  }

  return (
    <PluginContainer title='Turn on the tv in living room' subtitle={`Power: ${on ? 'ON' : 'OFF'}`} justifyContent="space-between">
