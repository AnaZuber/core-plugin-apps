...
export const WashingMachine: FC<Props> = ({ icon }) => {
  const theme = useCoreTheme();
  const classes = useStyles(theme);
  const [on, setOn] = useState(true);
  const [clicked, set] = useState(false)

  var Icon = IconRegistry['washingMachine'];
  ...
  return (
    <PluginContainer title='Turn on the washing machine in bathroom' subtitle={`Power: ${on ? 'ON' : 'OFF'}`} justifyContent="space-between">
